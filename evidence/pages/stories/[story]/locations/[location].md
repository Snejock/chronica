...
<script>
  ...
  $: if (q_story_name?.[0]?.story_nm) {
    breadcrumb?.set({ storyName: q_story_name[0].story_nm, locationName: q_location?.[0]?.canonical_nm });
  }
  ...
  // Лента локации, как и лента актёра, может растягиваться на недели —
  ...
  // Русское название страны по ISO-коду — через встроенный Intl.DisplayNames
  // (есть и в браузерах, и в Node при пререндере), собственный справочник не нужен.
  let _countryNames = null;
  try { _countryNames = new Intl.DisplayNames(['ru'], { type: 'region' }); } catch (e) { /* нет ICU — покажем код */ }
  function countryName(cc) {
    if (!cc) return '';
    try { return (_countryNames && _countryNames.of(cc.toUpperCase())) || cc; } catch (e) { return cc; }
  }

  // Человекочитаемый тип локации: сначала точный feature_code (GeoNames),
  // затем — грубый location_type из dds.h_locations.
  ...
  const TYPE_LABELS = { admin: 'Регион', city: 'Город', water: 'Акватория' };
  function typeLabel(row) {
    return FEATURE_LABELS[row.feature_code] || TYPE_LABELS[row.location_type] || '';
  }
  ...
  // Двухуровневая группировка: сначала по календарному дню (публикации с упоминанием
  // локации могут растягиваться на недели), внутри дня — по времени суток.
...
SELECT
    ...
    , region_nm
    , country_code
    , count(DISTINCT news_id) AS mentions_cnt
    , min(published_dttm) AS first_dttm
    , max(published_dttm) AS last_dttm
FROM dwh_pg_1.b_story_news_locations
WHERE story_id = ${params.story}
  AND location_id = '${params.location}'
GROUP BY location_id, canonical_nm, location_type, feature_code, region_nm, country_code
```
...
SELECT
    ...
    , feed_nm
    , feed_type
    -- страна источника новости, а не самой локации (см. source b_story_news_locations)
    , news_country_code AS country_code
    , city_nm
    , title_txt
    , summary_txt
    , image_url
...
# {locationRow.canonical_nm}

<!-- Шапка локации — текстовая, без фото (аналога s_actor_media у локаций нет):
     тип места + иерархия страна/регион. Для страны (PCLI) её собственный
     country_code не показываем — он дублировал бы заголовок. -->
<div class="not-prose mb-6">
  <p class="text-sm leading-relaxed" style="color:#57534e">
    {typeLabel(locationRow)}{#if locationRow.country_code && locationRow.feature_code !== 'PCLI'}&nbsp;·&nbsp;{flag(locationRow.country_code)} {countryName(locationRow.country_code)}{/if}{#if locationRow.region_nm}&nbsp;·&nbsp;{locationRow.region_nm}{/if}
  </p>
  <p class="text-xs mt-1" style="color:#a8a29e">
    {pluralize(Number(locationRow.mentions_cnt), 'упоминание', 'упоминания', 'упоминаний')} в сюжете&nbsp;·&nbsp;последнее {fmtDate(locationRow.last_dttm)}
  </p>
</div>
...
<div class="not-prose mt-4 p-6 rounded-xl" style="background:#f5f4f2; border:1px solid #e7e5e4">
  <p class="text-sm" style="color:#a8a29e">Публикаций с упоминанием этой локации в сюжете пока не найдено.</p>
</div>
...
<div class="not-prose mt-4 p-6 rounded-xl" style="background:#f5f4f2; border:1px solid #e7e5e4">
  <p class="text-sm" style="color:#a8a29e">Локация не найдена в этом сюжете.</p>
</div>
